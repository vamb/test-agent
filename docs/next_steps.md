...
| 优先级 | 工作项 | 目标 | 验收标准 |
|---:|---|---|---|
| 1 | Prompt Injection 与工具安全基线 | 对用户输入、RAG 文档和工具 observation 建立危险指令识别和权限边界测试 | 文档内容不能覆盖系统/开发者指令；高风险工具必须经过权限和确认策略 |
| 2 | 后台角色管理 UI | W32 后端已有 admin role 门禁，下一步让管理员能在后台查看用户并调整角色 | 不需要直接改数据库即可创建/维护 admin 用户；角色变更有审计记录 |
