...
10. 聊天界面体验增强：
   - 后端 SSE 从“步骤流 + 最终答案”补强为包含 `answer_delta` 的可感知答案流
   - 前端回答区支持 Markdown、列表、表格、代码块和引用块的基础渲染
   - 输入框支持自动增高、发送后清空、停止生成、快捷恢复上一条问题和更明确的聚焦状态
   - 消息列表支持自动滚动、用户上滑暂停自动跟随、回到最新按钮
   - 工具执行过程从裸事件列表优化为可折叠 timeline，主答案优先展示
11. 用户体系、聊天记录和 AI Memory 前置：
   - 先做用户注册、登录、登出、当前用户识别和可撤销 session
   - 再做聊天组、会话、消息和消息 artifacts，所有聊天数据绑定 `user_id`
   - Agent run 追加 `conversation_id`、输入消息和输出消息引用，便于回放和追踪
   - AI Memory 不直接等于聊天记录，先做会话摘要，再做长期偏好/事实记忆
12. 登录入口和聊天侧栏重整：
   - 已把登录/注册从聊天侧栏拆成独立入口页
   - 已将侧栏聚焦为用户状态、新建会话、聊天记录和管理台入口
   - 已隐藏服务状态信息，并把快捷对照收进折叠工具
   - 聊天顶部会显示当前会话标题和保存状态
...
| W21 | 21 | 聊天界面体验增强 | SSE 增加答案增量事件；聊天页支持 Markdown/结构化回答、自动滚动、停止生成、输入框自动增高、执行过程 timeline 和更清晰的事件/引用结果区 | 已完成第一版 |
| W22 | 22 | 用户体系后端第一版 | 新增用户、密码登录、HttpOnly session、登出和当前用户接口 | 已完成第一版 |
| W23 | 23 | 聊天组和聊天记录存储 | 新增聊天组、会话、消息、消息 artifacts；Agent run 可绑定用户和会话 | 已完成第一版 |
| W24 | 24 | AI Memory 前置设计 | 基于聊天记录先做会话摘要，再扩展长期记忆表 | 待开始 |
| W25 | 25 | 用户和聊天记录前端接入 | 聊天页接入登录/注册/登出、会话列表、新建会话和历史消息加载 | 已完成第一版 |
| W26 | 26 | 登录入口和聊天侧栏重整 | 未登录先进入账号界面；登录后侧栏只保留用户、会话、管理入口和折叠工具；聊天顶部显示当前会话和保存状态 | 已完成第一版 |
| W33 | 33 | Prompt Injection 和工具安全基线 | 已完成明显注入/绕过确认输入拦截、模型自确认高风险工具拦截、长期记忆不可信上下文注入和 system prompt 安全边界强化 | 已完成第一版 |
| W34 | 34 | 评测体系升级和 Langfuse eval 接入 | 已扩展本地 runner 安全断言字段，新增安全评测数据集，并补 LangGraph 确认恢复安全回归；后续同步到 Langfuse datasets/evals | 进行中 |

## 后续建议

| 优先级 | 工作项 | 目标 | 验收标准 |
|---:|---|---|---|
| 1 | 确认链路端到端回归演练 | 覆盖事件修订、来源核验、普通用户无权确认、admin 确认恢复、失败重试和刷新恢复 | 真实浏览器和 API 均能稳定完成或阻止对应路径；关键状态写入聊天记录和 agent_steps |
| 2 | 安全评测集扩充 | 把 Prompt Injection、越权确认、恶意 RAG 文档和 confirmed=true 绕过尝试加入自动评测 | 安全场景回归测试能在模型或 prompt 变更前稳定发现风险 |
| 3 | 用户体系和聊天记录存储 | 建立用户身份、聊天组、会话和消息持久化，为 AI Memory 打地基 | 用户只能访问自己的聊天数据；Agent run 可从聊天记录回放 |
| 4 | 聊天界面体验增强 | 把当前“步骤流 + 最终答案”升级为更像主流 chatbot 的实时回答、结构化渲染和输入体验 | 用户能看到答案逐段出现、工具过程可扫读、长回答格式清晰、请求可取消 |
| 5 | 前端确认面板增强 | 为真实业务工具展示更友好的参数摘要和影响范围，而不是只展示 JSON | 事件修订/来源核验确认面板能显示目标事件、字段变化、风险提示和跳转入口 |
| 6 | Langfuse datasets/evals 接入 | 暂后；把本地评测集和运行结果同步到 Langfuse | 本地评测可创建或更新 Langfuse dataset；评测运行能回写 scores/metadata |

### W33 后续加强项（待做）

| 编号 | 待做项 | 验收标准 |
|---:|---|---|
| S1 | 安全评测集扩充 | 已新增本地安全评测数据集，自动评测覆盖 Prompt Injection、越权确认、恶意 RAG 文档、长期记忆注入、工具 observation 注入和 `confirmed=true` 绕过；待同步 Langfuse |
| S2 | RAG/工具返回上下文隔离升级 | 已完成第一步：工具 observation 文本字段会生成不可信上下文 sidecar，references 标记 `untrusted/context_label`；待继续覆盖导入资料等更多入口 |
| S3 | 细粒度工具权限策略 | 只读、草案、写入、导入、知识库、向量工具有清晰角色权限；后端统一校验，不依赖模型判断 |
| S4 | 参数篡改和确认恢复安全测试 | 已覆盖确认恢复参数漂移、重复确认、模型自确认高风险工具和普通用户无权确认；待补过期确认、错误 run 归属和更细粒度资源权限测试 |
| S5 | 安全事件审计 | 已新增 `security_audit_logs` 和 recorder 写入/查询方法；手写 Loop 与 LangGraph 安全拦截会写审计；待继续接 Langfuse metadata 和后台查询入口 |
