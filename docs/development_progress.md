...
| 2026-07-30 | 完成 LangGraph 人工确认中断节点 W17：新增 `confirmation_required` 节点，遇到 `requires_confirmation=true` 或 `risk_level=high` 的工具且参数未带 `confirmed: true` 时暂停执行；同步调用返回确认提示并把 run 标记为 `waiting_for_user`，streaming 输出 `confirmation_required` 事件 | 后端 `python -m unittest tests.test_agent_workflow` 通过，9/9；后端 `python -m unittest tests.test_agent_workflow tests.test_agent_loop tests.test_api_agent tests.test_agent_worker` 通过，29/29；后端 `python -m unittest discover tests` 通过，81/81 | 下一步建议补确认后恢复执行入口，或接 Langfuse datasets/evals |
| 2026-07-30 | 完成确认后恢复执行入口 W18：新增 `AgentRunRecorder.claim_waiting_run`、`LangGraphAgentWorkflow.confirm_existing` 和 `POST /agent/runs/{run_id}/confirm`；确认恢复会读取 `skipped` confirmation step，把待确认工具参数补上 `confirmed: true` 后继续执行并覆盖为 completed step | 后端 `python -m unittest tests.test_agent_workflow` 通过，10/10；后端 `python -m unittest tests.test_agent_workflow tests.test_agent_loop tests.test_api_agent tests.test_agent_worker` 通过，30/30；后端 `python -m unittest discover tests` 通过，82/82 | 前端确认交互已进入 W19；后续接 Langfuse datasets/evals |
| 2026-07-30 | 完成确认恢复前端交互 W19：聊天页可识别 `confirmation_required` SSE 事件，展示待确认工具和参数；点击“确认并继续执行”会调用 `POST /agent/runs/{run_id}/confirm`，并把恢复后的答案、事件、引用、链接和步骤合并回原 assistant 消息 | 前端 `npm.cmd run build` 通过 | 下一步建议做 Langfuse datasets/evals 接入，或做一次 LangGraph 确认链路端到端联调 |
| 2026-07-30 | 完成 LangGraph 确认链路端到端联调 W20：新增默认关闭的 `AGENT_ENABLE_CONFIRMATION_PROBE` 本地确认探针；真实启动 `AGENT_WORKFLOW_ENGINE=langgraph` 后，浏览器完成 `confirmation_required -> 确认按钮 -> /agent/runs/{run_id}/confirm -> completed` 链路 | 后端 `python -m unittest tests.test_agent_workflow tests.test_api_agent tests.test_agent_loop` 通过，24/24；前端 `npm.cmd run build` 通过；真实 API run `6138723d-2b48-4422-bd51-23b27a31b2a9` 状态为 `completed`，step 参数 `confirmed=true` | 下一步建议接入真实高风险业务工具，优先做 Agent 驱动的事件修订/来源核验草案和对应前端确认体验 |
| 2026-07-31 | 推进 W34 本地安全评测和 W33 后续上下文隔离：评测 runner 支持 `expected_link_types`、`expected_security_categories`，新增安全评测数据集；工具 observation 文本字段增加不可信上下文 sidecar，references 标记 `untrusted/context_label` | 后端 `python -m unittest discover tests` 通过，103/103，跳过 1；安全评测覆盖 Prompt Injection、越权确认、恶意 RAG/记忆/工具 observation 注入和 `confirmed=true` 绕过 | 下一步建议继续接 Langfuse datasets/evals，或补参数篡改和确认恢复安全测试 |
| 2026-07-31 | 完成 W34 确认恢复安全回归第一步：LangGraph `confirm_existing` 改为使用 recorder 中保存的 pending 参数恢复执行，模型原始输出自带 `confirmed=true` 会被安全策略拦截 | 后端 `python -m unittest discover tests` 通过，106/106，跳过 1；新增参数漂移、重复确认、模型自确认高风险工具测试；HTTP 普通用户确认门禁测试继续通过 | 下一步建议补确认过期、run 归属/租户隔离和安全事件审计 |
| 2026-07-31 | 完成 W34/S5 安全事件审计第一版：新增 `security_audit_logs` schema、recorder 写入/查询方法和运行时建表兜底，手写 Loop 与 LangGraph 的安全拦截都会记录审计事件 | 后端 `python -m unittest discover tests` 通过，109/109，跳过 1；新增 recorder、Loop、LangGraph 和 schema 文件测试 | 下一步建议把安全审计同步到 Langfuse metadata，并补确认过期/租户归属测试 |
