...
| 2026-07-29 | 完成前端主流程改造：聊天页成为主入口；引入 `react-router-dom`；事件卡片跳转 `/events/:eventId`；详情页独立展示事件档案；补齐移动端适配；前端默认 API 切换到 `19000` | 前端 `npm.cmd run build` 通过；前端 `http://127.0.0.1:5174` 返回 200；后端 `http://127.0.0.1:19000/health` 正常 | 优先做 Agent 返回结构标准化、横向对照详情页、数据导入审核后台 |
...
当前状态：后端查询、Agent Loop、Function Calling、工具稳定性、模型观测、SSE 步骤流、运行取消、Redis 队列/Worker、processing ack、失败重试、死信队列、visibility timeout 回收、checkpoint 恢复、数据导入审核流、RAG 检索、事件管理、人工确认、React 聊天主界面、React Router 页面跳转、事件详情页和移动端适配均已完成 MVP；下一步优先进入 Agent 返回结构标准化、横向对照详情页、数据导入审核后台。
...
| 29 | Agent 返回结构标准化 | 前端/后端 | 待开始 | 第二阶段：Structured Output；第十六阶段：前后端通信 |
| 30 | 横向对照结果详情页 | 前端/后端 | 待开始 | 第四阶段：工具系统；业务可视化前端 |
| 31 | 数据导入审核后台 | 前端/后端 | 待开始 | 第十阶段：人工确认机制；第十二阶段：安全体系；平台化第一版 |

## 下一阶段执行计划

| 优先级 | 任务 | 目标 | 验收标准 | 对应 PDF 功能/章节 |
|---:|---|---|---|---|
| 1 | Agent 返回结构标准化 | 后端明确输出 `answer`、`references`、`links`，前端根据类型渲染事件、来源、对照卡片 | 前端不再依赖递归解析工具 observation；刷新和跳转后链接仍可用 | 第二阶段：Structured Output；第十六阶段：前后端通信 |
| 2 | 横向对照结果详情页 | 为一次对照查询提供独立页面，展示地区分栏、时间范围、事件列表和关联说明 | 点击聊天中的对照卡片可进入详情页，事件仍可继续跳转事件档案 | 第四阶段：工具系统；业务可视化前端 |
| 3 | 数据导入审核后台 | 接入导入批次创建、staging 查看、确认入库、驳回批次 | 可以通过前端完成一批历史事件的人工审核和入库 | 第十阶段：人工确认机制；第十二阶段：安全体系 |
| 4 | RAG 来源引用注入 | 把知识库检索结果注入 Agent 回答，并以引用卡片展示 | 回答中能看到来源标题、引用文本、可靠性和详情入口 | 第八阶段：RAG 知识库 |
| 5 | 会话历史持久化 | 用 `agent_runs` 支撑左侧会话列表和历史记录恢复 | 刷新页面后可重新打开历史问答 | 第七阶段：状态、记忆和上下文；第十一阶段：任务持久化 |
