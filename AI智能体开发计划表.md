...
| 3 | 用户体系和聊天记录存储 | 建立用户身份、聊天组、会话和消息持久化，为 AI Memory 打地基 | 第八阶段：记忆和上下文工程；第十二阶段：安全体系 |
| 4 | 聊天界面体验增强 | 把当前“步骤流 + 最终答案”升级为可感知的答案流、结构化渲染和更顺手的输入交互 | 第十六阶段：前后端通信；React 聊天页面 |
| 5 | 前端确认面板增强 | 针对真实业务工具展示目标事件、字段差异、影响范围和跳转入口，而不是只展示 JSON | 第十六阶段：前后端通信；平台化第一版 |
| 6 | 继续数据质量运营 | 按专题扩展真实历史事件，并用质量台账持续处理低置信、弱来源和重复候选 | 第十阶段：人工确认机制；平台化第一版 |
| 7 | Langfuse datasets/evals 和 RBAC | 暂后；等后端与配套前端功能更完整后再接 | 第十三阶段：评测体系；第十二阶段：安全体系 |
