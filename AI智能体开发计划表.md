...
| 下一步重点 | 提交 W14 改动、拆 LangGraph 细粒度节点、Langfuse datasets/evals 对接 |
...
| 5 | LangGraph 工作流 | 已完成迁移前置和三节点 pipeline；后续把 `execute_agent_loop` 拆成 decision、tool execution、confirmation、finish 细粒度节点 | 第六阶段：模型决策和固定工作流结合；第十一阶段：Checkpoint |
