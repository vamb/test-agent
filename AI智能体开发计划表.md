...
| 3 | 业务确认面板 | 已完成技术链路 | 现在确认面板偏 JSON 展示；真实事件修订、来源核验等高风险操作需要展示目标事件、字段 diff、影响范围和审计入口 | 第十阶段：人工确认机制；第十二阶段：安全体系 |
| 4 | Agent 驱动的数据修订工具 | 未开始 | 当前 Agent 主要读数据和回答；还不能生成事件修订草案、来源核验建议并在确认后调用管理 API 落库 | 第四阶段：工具系统；平台化第一版 |
| 5 | 完整 RBAC / 多租户权限 | 暂缓 | 已有登录和 HttpOnly session，但管理后台、知识库、工具和数据仍缺角色、租户、资源权限和策略落库 | 第十二阶段：安全体系 |
...
| W29 | 29 | Agent 事件修订草案工具 | 新增只生成草案的事件修订工具和确认后落库工具，复用现有管理 API 和审计日志 | Agent 可根据用户指令生成字段修订建议；未确认前不写库；确认后更新事件并记录 `event_change_logs`；前后端测试覆盖 | 待开始 |
| W30 | 30 | 来源核验和可靠度调整 Agent 工具 | 让 Agent 基于用户指令或知识库内容提出来源可靠度、主来源标记和 citation 修订建议 | 确认前只展示建议；确认后调用来源核验/编辑接口；后台事件详情能看到审计记录；回答可返回来源变更结果和事件链接 | 待开始 |
| W31 | 31 | 业务确认面板增强 | 针对事件修订、来源核验等真实业务工具展示目标事件、字段 diff、风险说明、影响范围和跳转入口 | 确认面板不再只展示 JSON；用户能看懂将要修改什么；确认、取消、失败、重试和恢复状态都清晰 | 待开始 |
| W32 | 32 | RBAC 和资源权限第一版 | 新增角色、权限策略、资源授权和管理接口权限校验，逐步替换 admin token | 权限由后端根据 session/role/resource 判断；普通用户不能访问管理写接口；聊天、知识库、导入批次和工具调用按用户或租户隔离 | 待开始 |
...
| 3 | Agent 驱动的事件修订草案 | 让 Agent 生成事件字段修订建议，确认后调用管理 API 落库并写审计 | 第四阶段：工具系统；第十阶段：人工确认机制 |
| 4 | Agent 驱动的来源核验/可靠度调整 | 让 Agent 基于用户指令或知识库内容提出来源可靠度和 citation 修订建议，确认后执行 | 第八阶段：引用来源；平台化第一版 |
